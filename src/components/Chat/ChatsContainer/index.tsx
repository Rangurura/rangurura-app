...
const ChatsContainer = ()=>{
    ...
    const chats = [
        ...
        },
        {
            ...
            user: "Ranjan Maari",
            time: "11:30",
            latestMessage: "Thank you for scheduling my appointment. I confirm that I will be present tomorrow at the designated time",
            id: 5,
            status: "Online"
        },
        {
    ...
    return(
        <div className="w-[34%] h-full bg-white rounded-[1rem]">
            <div className="w-full h-16 rounded-t-[1rem] border-b border-b-[1px] border-b-[#ccc] flex flex-col items-center justify-center">
                <input type="text" name="search" id="" placeholder="Search" className="w-4/5 h-8 rounded-[0.8rem] text-[80%] border-none outline-none bg-[#9696961d] pl-5" />
            </div>
            <div className="w-full h-[90%] flex flex-col items-center pt-3">
                {chats.map(chat=>{
                    return(
                        <div className={`w-full flex justify-center hover:bg-[#E8F3FF] cursor-pointer ${chat.id === activeChatID ? "bg-[#E8F3FF]" : "" }`} onClick={()=> {localStorage.setItem("activeChat", JSON.stringify(chat)); setActiveChatID(chat.id)}}>
                            <div className="w-[85%] flex justify-between items-start border-b border-b-[1px] border-b-[#E5E5E5] py-2">
                                ...
                                <div className="w-[45%] h-full flex flex-col gap-2 mr-[4rem]">
                                    <h5 className="text-[90%] font-bold text-[#283E58]">{chat.user}</h5>
                                    <h5 className="text-[80%] text-[#969696]">{chat.latestMessage && chat.latestMessage.length > 20 ? `${chat.latestMessage.slice(0,18)} . . .` : chat.latestMessage.length < 20 && chat.latestMessage.length != 0?  chat.latestMessage : "No message sent"}</h5>
                                </div>
